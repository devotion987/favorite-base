...
/**
 * Purpose:这个类提供了一些根据类的class文件位置来定位的方法。
 *
 * @since 1.0
 */
public class Path {
    ...
    public static String getPathFromClass(Class<?> cls) throws IOException {
        ...
        if (url != null) {
            ...
            // System.out.println("Jar root Path:"+path);
        }
        return path;
    }
    ...
    private static URL getClassLocationURL(final Class<?> cls) {
        ...
        if (pd != null) {
            ...
            try {
                // 在windows上获取得到的字符串包含“％20”，需要替换成原来的空格
    ...
    /**
     * 该方法获取类所在的根路径, 如果是以class形式存在，则它获取到classes目录的上一级目录
     * 如果是以jar的形式存在，则它获取jar所在的目录
     *
     * @param cls
     * @return
     */
    public static String getRootPath(Class<?> cls) throws IOException {
        ...
        if (path.lastIndexOf("jar") == (path.length() - 3)) {// 说明是以jar包存在
            File jarFile = new File(path);
            // 获取Jar包当前所在路径
            // System.out.println("cls is in jar,it's root path
            // is:"+jarFile.getParent());
            return jarFile.getParent();
        } else {
            String packagename = cls.getPackage().getName();
            StringTokenizer tokenizer = new StringTokenizer(packagename, ".", false);
            int num = tokenizer.countTokens() + 1;
            for (int i = 0; i < num - 1; i++) {// TODO：其实还需要考虑没有包名的类的情况
                relative += "../";
            }
    ...
    public static String getRootRelativePath(String relativePath, Class<?> cls) throws IOException {
        ...
        // getRootRelativePath("+relativePath+","+cls.getName()+"):"+path);
