...
import com.wugy.spring.annotation.aop.proxy.ProxyManager;
import com.wugy.spring.annotation.ioc.ServiceImpl;
import com.wugy.spring.annotation.mvc.service.ClassScanner;
import com.wugy.spring.annotation.tx.TransactionProxy;
import com.wugy.spring.plugin.PluginProxy;
import com.wugy.spring.utils.BeanHelper;
import com.wugy.spring.utils.ClassHelper;
import com.wugy.spring.utils.FrameworkConstant;
import com.wugy.spring.utils.InstanceFactory;
...
public class AopHelper {
	...
	private static void addPluginProxy(Map<Class<?>, List<Class<?>>> proxyMap) throws Exception {
		...
		List<Class<?>> pluginProxyClassList = classScanner.getClassListBySuper(FrameworkConstant.PLUGIN_PACKAGE,
				PluginProxy.class);
	...
	private static void addAspectProxy(Map<Class<?>, List<Class<?>>> proxyMap) throws Exception {
		...
		// 添加插件包下所有的切面类
		aspectProxyClassList
				.addAll(classScanner.getClassListBySuper(FrameworkConstant.PLUGIN_PACKAGE, AspectProxy.class));
		// 排序切面类
	...
	private static void addTransactionProxy(Map<Class<?>, List<Class<?>>> proxyMap) {
		...
		List<Class<?>> serviceClassList = ClassHelper.getClassListByAnnotation(ServiceImpl.class);
	...
	private static void sortAspectProxyClassList(List<Class<?>> proxyClassList) {
		...
		Collections.sort(proxyClassList, new Comparator<Class<?>>() {
			...
			private int getOrderValue(Class<?> aspect) {
				return aspect.getAnnotation(AspectOrder.class) != null ? aspect.getAnnotation(AspectOrder.class).value()
						: 0;
			}
		});
	}
