...
import com.wugy.spring.annotation.ioc.Inject;
import com.wugy.spring.annotation.ioc.ServiceImpl;

/**
 * 初始化 IOC 容器
 *
 * @author devotion 2016年9月2日14:21:26
 */
public class IocHelper {
						...
						if (beanField.isAnnotationPresent(Inject.class)) {
							...
							if (implementClass != null) {
								...
								if (implementInstance != null) {
									beanField.setAccessible(true); // 将字段设置为
																	// public
									beanField.set(beanInstance, implementInstance); // 设置字段初始值
								} else {
									throw new RuntimeException("依赖注入失败！类名：" + beanClass.getSimpleName() + "，字段名："
	...
	public static Class<?> findImplementClass(Class<?> interfaceClass) {
		...
		// 判断接口上是否标注了 Impl 注解
		if (interfaceClass.isAnnotationPresent(ServiceImpl.class)) {
			// 获取强制指定的实现类
			implementClass = interfaceClass.getAnnotation(ServiceImpl.class).value();
		} else {
			// 获取该接口所有的实现类
