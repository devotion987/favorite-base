...
class Util {
	 ...
	 * The class loader reference is never {@code null}, because {@code null}
	 ...
	 * making changes keep the code in sync.
	 */
	private static Wrapper findWrapper(Class<?> clazz, List<Wrapper> wrappers, String name, Class<?>[] paramTypes,
			Object[] paramValues) {
		...
		for (Wrapper w : wrappers) {
			...
			if (!(paramCount == mParamCount || (w.isVarArgs() && paramCount >= mParamCount))) {
