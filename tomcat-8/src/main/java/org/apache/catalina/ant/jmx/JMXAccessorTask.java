 ...
 * <em>jmx.server</em>
 * </p>
 * All JMXAccessorXXXTask support the attribute <em>if</em> and <em>unless</em>.
 * With <em>if</em> the task is only execute when property exist and with
 * <em>unless</em> when property not exists. <br>
...
public class JMXAccessorTask extends BaseRedirectorHelperTask {
	 ...
	 * prefix (<code>resultproperty.0-array.length</code> and store the result
	 * array length at <code>resultproperty.length</code>. Other option is that
	 * you delimit your result with a delimiter (java.util.StringTokenizer is
	 * used).
	 *
