...
/**
 *
 * The order interceptor guarantees that messages are received in the same order
 * they were sent. This interceptor works best with the ack=true setting. <br>
 * There is no point in using this with the replicationMode="fastasynchqueue" as
 * this mode guarantees ordering.<BR>
 * If you are using the mode ack=false replicationMode=pooled, and have a lot of
 * concurrent threads, this interceptor can really slow you down, as many
 * messages will be completely out of order and the queue might become rather
 * large. If this is the case, then you might want to set the value
 * OrderInterceptor.maxQueue = 25 (meaning that we will never keep more than 25
 * messages in our queue) <br>
 ...
 * OrderInteceptor.maxQueue=&lt;max queue size&gt; - how much can the queue grow
 * to ensure ordering. This setting is useful to avoid OutOfMemoryErrors
 * <b>default=Integer.MAX_VALUE</b><br>
 * OrderInterceptor.forwardExpired=&lt;boolean&gt; - this flag tells the
 * interceptor what to do when a message has expired or the queue has grown
 * larger than the maxQueue value. true means that the message is sent up the
 * stack to the receiver that will receive and out of order message false means,
 * forget the message and reset the message counter. <b>default=true</b>
 *
 *
 * @version 1.1
 */
public class OrderInterceptor extends ChannelInterceptorBase {
	protected static final StringManager sm = StringManager.getManager(OrderInterceptor.class.getPackage().getName());
