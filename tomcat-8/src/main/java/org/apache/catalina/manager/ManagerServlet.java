 ...
 * application deployment descriptor. However, this requirement can be relaxed
 * during testing.
 * <p>
 * This servlet examines the value returned by <code>getPathInfo()</code> and
 * related query parameters to determine what action is being requested. The
 * following actions and parameters (starting after the servlet path) are
 * supported:
 * <ul>
 * <li><b>/deploy?config={config-url}</b> - Install and start a new web
 * application, based on the contents of the context configuration file found at
 * the specified URL. The <code>docBase</code> attribute of the context
 * configuration file is used to locate the actual WAR or directory containing
 * the application.</li>
 * <li><b>/deploy?config={config-url}&amp;war={war-url}/</b> - Install and start
 * a new web application, based on the contents of the context configuration
 * file found at <code>{config-url}</code>, overriding the <code>docBase</code>
 * attribute with the contents of the web application archive found at
 * <code>{war-url}</code>.</li>
 * <li><b>/deploy?path=/xxx&amp;war={war-url}</b> - Install and start a new web
 * application attached to context path <code>/xxx</code>, based on the contents
 * of the web application archive found at the specified URL.</li>
 * <li><b>/list</b> - List the context paths of all currently installed web
 * applications for this virtual host. Each context will be listed with the
 * following format <code>path:status:sessions</code>. Where path is the context
 * path. Status is either running or stopped. Sessions is the number of active
 * Sessions.</li>
 * <li><b>/reload?path=/xxx</b> - Reload the Java classes and resources for the
 * application at the specified path.</li>
 * <li><b>/resources?type=xxxx</b> - Enumerate the available global JNDI
 * resources, optionally limited to those of the specified type (fully qualified
 * Java class name), if available.</li>
 ...
 * web application attached to context path <code>/xxx</code> for this virtual
 * host.</li>
 * <li><b>/expire?path=/xxx&amp;idle=mm</b> - Expire sessions for the context
 * path <code>/xxx</code> which were idle for at least mm minutes.</li>
 * <li><b>/sslConnectorCiphers</b> - Display diagnostic info on SSL/TLS ciphers
 * that are currently configured for each connector.
 * <li><b>/start?path=/xxx</b> - Start the web application attached to context
 * path <code>/xxx</code> for this virtual host.</li>
 * <li><b>/stop?path=/xxx</b> - Stop the web application attached to context
 * path <code>/xxx</code> for this virtual host.</li>
 * <li><b>/threaddump</b> - Write a JVM thread dump.</li>
 * <li><b>/undeploy?path=/xxx</b> - Shutdown and remove the web application
 * attached to context path <code>/xxx</code> for this virtual host, and remove
 * the underlying WAR file or document base directory. (<em>NOTE</em> - This is
 * only allowed if the WAR file or document base is stored in the
 * <code>appBase</code> directory of this host, typically as a result of being
 * placed there via the <code>/deploy</code> command.</li>
 ...
 * application deployed with path <code>/xxx</code> to an appropriately named
 * context.xml file in the <code>xmlBase</code> for the associated Host.</li>
 * </ul>
 * <p>
 * Use <code>path=/</code> for the ROOT context.
 * </p>
 * <p>
 * The syntax of the URL for a web application archive must conform to one of
 * the following patterns to be successfully deployed:
 * </p>
 * <ul>
 * <li><b>file:/absolute/path/to/a/directory</b> - You can specify the absolute
 * path of a directory that contains the unpacked version of a web application.
 * This directory will be attached to the context path you specify without any
 * changes.</li>
 * </ul>
 * <p>
 * <b>NOTE</b> - Attempting to reload or remove the application containing this
 * servlet itself will not succeed. Therefore, this servlet should generally be
 * deployed as a separate web application within the virtual host to be managed.
 * <p>
 * The following servlet initialization parameters are recognized:
 * <ul>
 * <li><b>debug</b> - The debugging detail level that controls the amount of
 * information that is logged by this servlet. Default is zero.
 * </ul>
 *
...
public class ManagerServlet extends HttpServlet implements ContainerServlet {
	...
	protected synchronized void save(PrintWriter writer, String path, StringManager smClient) {
		...
		if ((path == null) || path.length() == 0 || !path.startsWith("/")) {
			...
			try {
				mBeanServer.invoke(storeConfigOname, "store", new Object[] { context },
	 ...
	 * @param tag
	 *            Tag to be associated with the webapp
	 * @param update
	 *            Flag that indicates that any existing app should be replaced
	 * @param request
	 *            Servlet request we are processing
	 * @param smClient
	 *            i18n messages using the locale of the client
	 */
	protected synchronized void deploy(PrintWriter writer, ContextName cn, String tag, boolean update,
	 ...
	 * @param update
	 *            true to override any existing webapp on the path
	 * @param smClient
	 *            i18n messages using the locale of the client
	 */
	 ...
	 * path. Displays a profile of session thisAccessedTime listing number of
	 * sessions for each 10 minute interval up to 10 hours.
	 *
	 * @param writer
	 *            Writer to render to
	 * @param cn
	 *            Name of the application to list session information for
	 * @param idle
	 ...
	 * @param smClient
	 *            The StringManager used to construct i18n messages based on the
	 ...
	 *
	 * @param src
	 *            File object representing the source
	 * @param dest
	...
	protected Map<String, Set<String>> getConnectorCiphers() {
		...
		for (Connector connector : connectors) {
			...
			if (Boolean.TRUE.equals(connector.getProperty("SSLEnabled"))) {
				String[] ciphersUsed = (String[]) connector.getProperty("ciphersUsed");
