...
public class InternalAprInputBuffer extends AbstractInputBuffer<Long> {
	...
	private void skipLine(int start) throws IOException {
		...
		if (log.isDebugEnabled()) {
			log.debug(sm.getString("iib.invalidheader",
					new String(buf, start, lastRealByte - start + 1, StandardCharsets.ISO_8859_1)));
		}
	}
	...
	protected boolean fill(boolean block) throws IOException {
		...
		if (nRead > 0) {
			...
			if (log.isDebugEnabled()) {
				log.debug(sm.getString("iib.apr.sslGeneralError", Long.valueOf(socket), wrapper));
			}
			return false;
			...
			throw new IOException(sm.getString("iib.failedread.apr", Integer.valueOf(-nRead)));
		}
	}
