...
/**
 * An implementation of <tt>javax.servlet.jsp.tagext.PageData</tt> which builds
 * the XML view of a given page.
 *
 * The XML view is built in two passes:
 *
 * During the first pass, the FirstPassVisitor collects the attributes of the
 * top-level jsp:root and those of the jsp:root elements of any included pages,
 * and adds them to the jsp:root element of the XML view. In addition, any
 * taglib directives are converted into xmlns: attributes and added to the
 * jsp:root element of the XML view. This pass ignores any nodes other than
 * JspRoot and TaglibDirective.
 *
...
class PageDataImpl extends PageData implements TagConstants {
	...
	/*
	 * First-pass Visitor for JspRoot nodes (representing jsp:root elements) and
	 * TablibDirective nodes, ignoring any other nodes.
	 *
	 * The purpose of this Visitor is to collect the attributes of the top-level
	 * jsp:root and those of the jsp:root elements of any included pages, and
	 * add them to the jsp:root element of the XML view. In addition, this
	 * Visitor converts any taglib directives into xmlns: attributes and adds
	 * them to the jsp:root element of the XML view.
	 */
	private static class FirstPassVisitor extends Node.Visitor implements TagConstants {
		...
		public void visit(Node.Root n) throws JasperException {
			...
			if (n == root) {
				 ...
				 * Top-level page.
				 *
				 * Add xmlns:jsp="http://java.sun.com/JSP/Page" attribute only
				...
				if (pageInfo.isJspPrefixHijacked()) {
					 ...
					 * use the 'jsp' prefix. Therefore, create a new prefix (one
					 * that is unique across the translation unit) for use by
					 * the 'id' attribute, and bind it to the JSP namespace
					 */
					jspIdPrefix += "jsp";
		...
		public void visit(Node.TaglibDirective n) throws JasperException {
			...
			if (attrs != null) {
				 ...
				 * specified attribute is already contained in the list: This is
	...
	/*
	 * Second-pass Visitor responsible for producing XML view and assigning each
	 * element a unique jsp:id attribute.
	 */
	private static class SecondPassVisitor extends Node.Visitor implements TagConstants {
		 ...
		 * Constructor
		 */
		public SecondPassVisitor(Node.Root root, StringBuilder buf, Compiler compiler, String jspIdPrefix) {
			this.root = root;
			this.buf = buf;
		 ...
		 * document, and since XML allows only single-value attributes, the
		 * values of multiple import attributes must be combined into one,
		 * separated by comma.
		 *
		 * If the given page directive contains just 'contentType' and/or
		 * 'pageEncoding' attributes, we ignore it, as we've already appended a
		 * page directive containing just these two attributes.
		 */
		private void appendPageDirective(Node.PageDirective n) {
			...
			for (int i = 0; i < len; i++) {
				...
				if ("import".equals(attrName) || "contentType".equals(attrName) || "pageEncoding".equals(attrName)) {
					 ...
					 * down, and its 'pageEncoding' and 'contentType' attributes
					 * are ignored, since we've already appended a new page
		 ...
		 * attributes.
		 *
		 * The value of the 'pageEncoding' attribute is hard-coded to UTF-8,
		 * whereas the value of the 'contentType' attribute, which is identical
		 * to what the container will pass to ServletResponse.setContentType(),
		 * is derived from the pageInfo.
		 */
		 ...
		 *
		 * If the given tag directive contains just a 'pageEncoding' attributes,
		 * we ignore it, as we've already appended a tag directive containing
		 * just this attributes.
		 */
