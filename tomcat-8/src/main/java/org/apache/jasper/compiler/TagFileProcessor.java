	...
	private static class TagFileDirectiveVisitor extends Node.Visitor {
		...
		private static final JspUtil.ValidAttribute[] tagDirectiveAttrs = { new JspUtil.ValidAttribute("display-name"),
				...
				new JspUtil.ValidAttribute("import"), new JspUtil.ValidAttribute("deferredSyntaxAllowedAsLiteral"), // JSP
																													// 2.1
				new JspUtil.ValidAttribute("trimDirectiveWhitespaces"), // JSP
																		// 2.1
		...
		private static final JspUtil.ValidAttribute[] attributeDirectiveAttrs = {
				...
				new JspUtil.ValidAttribute("type"), new JspUtil.ValidAttribute("deferredValue"), // JSP
																									// 2.1
			...
			synchronized (rctxt) {
				...
				try {
					...
					if (wrapper == null) {
						...
						// Compilation may be nested in which case the old
						// tagJar
					...
					try {
						...
						if (tagIns instanceof JspSourceDependent) {
							Iterator<Entry<String, Long>> iter = ((JspSourceDependent) tagIns).getDependants()
									.entrySet().iterator();
							while (iter.hasNext()) {
