...
class Validator {
	...
	private static class DirectiveVisitor extends Node.Visitor {
		 ...
		 * page directive @param pageDir The page directive node
		 *
		 * @throws JasperException in case of page encoding mismatch
		 */
	...
	private static class ValidateVisitor extends Node.Visitor {
		...
		private static final JspUtil.ValidAttribute[] includeActionAttrs = { new JspUtil.ValidAttribute("page", true),
		...
		private static final JspUtil.ValidAttribute[] setPropertyAttrs = { new JspUtil.ValidAttribute("name", true),
				new JspUtil.ValidAttribute("property", true), new JspUtil.ValidAttribute("value", false),
		...
		private static final JspUtil.ValidAttribute[] useBeanAttrs = { new JspUtil.ValidAttribute("id", true),
				...
				new JspUtil.ValidAttribute("type"), new JspUtil.ValidAttribute("beanName", false) };
		...
		private static final JspUtil.ValidAttribute[] plugInAttrs = { new JspUtil.ValidAttribute("type", true),
				...
				new JspUtil.ValidAttribute("height", false), new JspUtil.ValidAttribute("hspace"),
				new JspUtil.ValidAttribute("jreversion"), new JspUtil.ValidAttribute("name"),
				new JspUtil.ValidAttribute("vspace"), new JspUtil.ValidAttribute("width", false),
		...
		private static final JspUtil.ValidAttribute[] attributeAttrs = { new JspUtil.ValidAttribute("name", true),
				new JspUtil.ValidAttribute("trim"), new JspUtil.ValidAttribute("omit") };
		...
		public void visit(Node.ParamAction n) throws JasperException {
			...
			n.setValue(getJspAttribute(null, "value", null, null, n.getAttributeValue("value"), n, null, false));
			visitBody(n);
		}
		...
		public void visit(Node.IncludeAction n) throws JasperException {
			...
			n.setPage(getJspAttribute(null, "page", null, null, n.getAttributeValue("page"), n, null, false));
			visitBody(n);
		}
		...
		public void visit(Node.SetProperty n) throws JasperException {
			...
			n.setValue(getJspAttribute(null, "value", null, null, value, n, null, false));
		...
		public void visit(Node.UseBean n) throws JasperException {
			...
			Node.JspAttribute jattr = getJspAttribute(null, "beanName", null, null, n.getAttributeValue("beanName"), n,
					null, false);
			n.setBeanName(jattr);
		...
		// type can't be null after initial test
		@Override
		public void visit(Node.PlugIn n) throws JasperException {
			...
			Node.JspAttribute width = getJspAttribute(null, "width", null, null, n.getAttributeValue("width"), n, null,
					false);
			n.setWidth(width);

			Node.JspAttribute height = getJspAttribute(null, "height", null, null, n.getAttributeValue("height"), n,
					null, false);
			n.setHeight(height);
		...
		public void visit(Node.NamedAttribute n) throws JasperException {
			...
			n.setOmit(getJspAttribute(null, "omit", null, null, n.getAttributeValue("omit"), n, null, false));
			visitBody(n);
		}
		...
		// tagInfo can't be null after initial test
		@Override
		...
		public void visit(Node.JspElement n) throws JasperException {
			...
			// Exception will have been thrown above
		...
		private void checkXmlAttributes(Node.CustomTag n, Node.JspAttribute[] jspAttrs,
			...
			for (int i = 0; attrs != null && i < attrs.getLength(); i++) {
				...
				boolean deferredSyntaxAllowedAsLiteral = pageInfo.isDeferredSyntaxAllowedAsLiteral()
						|| libraryVersion < 2.1;
				...
				if (!elExpression && el != null) {
					...
					if (it.hasNext()) {
						textAttributeValue = ((Text) it.next()).getText();
					} else {
						textAttributeValue = "";
					}
				} else {
						...
						if (tldAttr.canBeRequestTime() || tldAttr.isDeferredMethod() || tldAttr.isDeferredValue()) { // JSP
							...
							if (!expression) {
								...
								if (!deferred && !tldAttr.canBeRequestTime()) {
									// Only deferred expressions are allowed for
									// this attribute
									err.jspError(n, "jsp.error.attribute.custom.non_rt_with_expr", tldAttr.getName());
								}
		 ...
		 *
		 * @param el
		 *            EL expression, if already parsed by the caller (so that we
		...
		private boolean isExpression(Node n, String value, boolean checkDeferred) {
			...
			if (!runtimeExpression && !pageInfo.isELIgnored()) {
				Iterator<ELNode> nodes = ELParser.parse(value, pageInfo.isDeferredSyntaxAllowedAsLiteral()).iterator();
				while (nodes.hasNext()) {
					ELNode node = nodes.next();
					if (node instanceof ELNode.Root) {
						if (((ELNode.Root) node).getType() == '$') {
		...
		private FunctionMapper getFunctionMapper(ELNode.Nodes el) throws JasperException {
			...
			class MapperELVisitor extends ELNode.Visitor {
				...
				// c can't be null after catch block
				@Override
