...
class Generator {
	...
	private static final String VAR_EXPRESSIONFACTORY = System
			.getProperty("org.apache.jasper.compiler.Generator.VAR_EXPRESSIONFACTORY", "_el_expressionfactory");
	private static final String VAR_INSTANCEMANAGER = System
			.getProperty("org.apache.jasper.compiler.Generator.VAR_INSTANCEMANAGER", "_jsp_instancemanager");
	 ...
	 */
	private static final boolean STRICT_GET_PROPERTY = Boolean
			.parseBoolean(System.getProperty("org.apache.jasper.compiler.Generator.STRICT_GET_PROPERTY", "true"));
	...
	private void genPreambleClassVariableDeclarations() {
		if (isPoolingEnabled && !tagHandlerPoolNames.isEmpty()) {
			for (int i = 0; i < tagHandlerPoolNames.size(); i++) {
				out.printil(
						"private org.apache.jasper.runtime.TagHandlerPool " + tagHandlerPoolNames.elementAt(i) + ";");
			}
			out.println();
		}
	...
	private class GenerateVisitor extends Node.Visitor {
		 ...
		 * Constructor.
		 */
		public GenerateVisitor(boolean isTagFile, ServletWriter out, ArrayList<GenBuffer> methodsBuffered,
		...
		private String attributeValue(Node.JspAttribute attr, boolean encode, Class<?> expectedType) {
			...
			if (attr.isExpression()) {
				...
				if (encode) {
					return "org.apache.jasper.runtime.JspRuntimeLibrary.URLEncode(" + v
							+ ", request.getCharacterEncoding())";
				}
				return v;
		...
		public void visit(Node.ELExpression n) throws JasperException {
			...
			if (!pageInfo.isELIgnored() && (n.getEL() != null)) {
				...
				out.printil("out.write(" + quote(n.getType() + "{" + n.getText() + "}") + ");");
			}
			n.setEndJavaLine(out.getJavaLine());
		}
		 ...
		 * subelements. For each &lt;param&gt; element, if its value is
		 * specified via a Named Attribute (&lt;jsp:attribute&gt;), generate the
		 * code to evaluate those bodies first.
		 * <p>
				...
				for (int i = 0; i < subelements.size(); i++) {
					...
					if (n instanceof Node.ParamAction) {
						...
						for (int j = 0; (paramSubElements != null) && (j < paramSubElements.size()); j++) {
							...
							if (m instanceof NamedAttribute) {
								generateNamedAttributeValue((NamedAttribute) m);
							}
						}
					}
				}
		...
		public void visit(Node.JspElement n) throws JasperException {
			...
			if (subelements != null) {
				for (int i = 0; i < subelements.size(); i++) {
					...
					if (!(subelem instanceof NamedAttribute)) {
						hasBody = true;
						break;
					}
				}
		...
		private TagHandlerInfo getTagHandlerInfo(Node.CustomTag n) throws JasperException {
			Hashtable<String, TagHandlerInfo> handlerInfosByShortName = handlerInfos.get(n.getPrefix());
		...
		private String convertString(Class<?> c, String s, String attrName, Class<?> propEditorClass,
			...
			if (propEditorClass != null) {
				...
				String className = c.getCanonicalName();
				return "(" + className
						+ ")org.apache.jasper.runtime.JspRuntimeLibrary.getValueFromPropertyEditorManager(" + className
						+ ".class, \"" + attrName + "\", " + quoted + ")";
			}
		}
		 ...
		 */
		public String generateNamedAttributeValue(NamedAttribute n) throws JasperException {
			...
			if (body != null) {
				...
				if (body.size() == 1) {
					...
					if (bodyElement instanceof Node.TemplateText) {
						...
						out.printil("java.lang.String " + varName + " = "
								+ quote(((Node.TemplateText) bodyElement).getText()) + ";");
					}
				}
				...
				if (!templateTextOptimization) {
					out.printil("out = _jspx_page_context.pushBody();");
					visitBody(n);
					out.printil("java.lang.String " + varName + " = " + "((javax.servlet.jsp.tagext.BodyContent)"
							+ "out).getString();");
					out.printil("out = _jspx_page_context.popBody();");
				}
			} else {
		 ...
		 */
		public String generateNamedAttributeJspFragment(NamedAttribute n, String tagHandlerVar)
				throws JasperException {
			String varName = n.getTemporaryVariableName();
	...
	private static void generateLocalVariables(ServletWriter out, Node n) throws JasperException {
		...
		} else if (n instanceof NamedAttribute) {
			ci = ((NamedAttribute) n).getChildInfo();
		} else {
	...
	public static void generate(ServletWriter out, Compiler compiler, Node.Nodes page) throws JasperException {
		...
		if (gen.ctxt.isTagFile()) {
			...
			page.visit(
					gen.new GenerateVisitor(gen.ctxt.isTagFile(), out, gen.methodsBuffered, gen.fragmentHelperClass));
	...
	private static class FragmentHelperClass {
		...
		public void generatePreamble() {
			...
			out.println();
			out.printil("public " + className + "( int discriminator, javax.servlet.jsp.JspContext jspContext, "
